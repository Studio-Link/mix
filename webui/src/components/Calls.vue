<template>
    <div v-if="State.user.value.calling">
                ...
                <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                    <DialogPanel
                        ...
                        <div class="absolute top-0 right-0 hidden pt-4 pr-4 sm:block">
                            <button type="button" @click="api.call_delete(State.user.value.id)"
                        ...
                        <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                            <button type="button"
                                ...
                                @click="api.call_delete(State.user.value.id)">Cancel</button>
                        </div>
                    </DialogPanel>
                </div>
            </div>
    ...
    <div v-if="State.user.value.host" v-for="user in State.calls.value" as="template">
        <Dialog class="relative z-20" :open=true>
            <div class="fixed inset-0 bg-gray-500/75 transition-opacity" />
